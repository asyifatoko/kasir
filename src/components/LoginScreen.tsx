...
import { Crown, Shield, Coins, Lock, Eye, EyeOff, Sparkles, AlertCircle, Mail, WifiOff } from "lucide-react";
...
export default function LoginScreen({ onLoginSuccess }: LoginScreenProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col items-center justify-center p-4 md:p-8 relative overflow-hidden" id="pos-login-page">
      ...
      <div className="w-full max-w-lg space-y-8 z-10">
        <div className="text-center space-y-3">
          ...
          <div>
            <h1 className="text-2xl font-black tracking-tight text-white uppercase">SISTEM KASIR ASYIFA</h1>
        ...
        <div className="bg-slate-950/80 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 md:p-8 shadow-2xl space-y-6">

          {!useOfflineFallback ? (
            <form onSubmit={handleOnlineLogin} className="space-y-6">
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-400 block tracking-wider uppercase">Email</label>
                <div className="relative">
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-3 bg-slate-900 border border-slate-800 rounded-2xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-xs font-semibold text-white tracking-wide"
                  />
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-400 block tracking-wider uppercase">Kata Sandi</label>
                <div className="relative">
                  <input
                    ...
                    className="w-full pl-10 pr-12 py-3 bg-slate-900 border border-slate-800 rounded-2xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-xs font-semibold text-white tracking-wide"
                  />
            ...
            <form onSubmit={handleOfflineLogin} className="space-y-6">
              ...
              <div className="space-y-3">
                <label className="text-xs font-bold text-slate-400 block tracking-wider uppercase">Pilih Tingkat Akses:</label>
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-start gap-4 p-3.5 rounded-2xl border text-left transition-all duration-200 cursor-pointer ${
                          isSelected ? `${cfg.bgLight} border-${cfg.role === 'owner' ? 'amber-500' : cfg.role === 'admin' ? 'emerald-500' : 'blue-500'} shadow-lg` : "bg-slate-900/40 border-slate-800/80 hover:bg-slate-900/80"
                        }`}
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-[10px] text-slate-400 mt-1 leading-relaxed">{cfg.desc}</p>
                        </div>
                      </button>
                    );
                  })}
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-400 tracking-wider uppercase">Kata Sandi (Password):</label>
                <div className="relative">
                  <input
                    type={showPassword ? "text" : "password"}
                    required
